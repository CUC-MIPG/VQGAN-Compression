...
class DualGrainFeatureRouter(nn.Module):
    def __init__(self, num_channels, normalization_type="none", gate_type="1layer-fc"):
        ...
        print('Success create DualRouter')


    def forward(self, h_fine, h_coarse, entropy=None):
        h_fine = self.feature_norm_fine(h_fine) # Note: 对不同分辨率的特征做归一化，保证稳定
        h_coarse = self.feature_norm_coarse(h_coarse) # Note: 对不同分辨率的特征做归一化，保证稳定

        avg_h_fine = self.gate_pool(h_fine) # Note: 对细粒度分辨率进行池化，使其分辨率与粗粒度相同，例如 4*4 -> 2*2
        h_logistic = torch.cat([h_coarse, avg_h_fine], dim=1).permute(0,2,3,1)
        # Note: [B C H W] + [B C H W] -> [B H W 2*C]
