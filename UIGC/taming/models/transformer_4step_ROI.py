...
class Transformer_stage1_serial(Transformer_stage1_parallel):
    ...
    def test_step(self, batch, batch_idx):
        ...
        roi_pos = mask_info['seg_info']['main_box']
        ...
        roi_img_pred_rec = img_pred_rec[:, :, roi_pos[1]:roi_pos[3], roi_pos[0]:roi_pos[2]]
        ...
        img_rec = img_rec / 2.0 + 0.5
        img_pred_rec = img_pred_rec / 2.0 + 0.5
        roi_img = roi_img / 2.0 + 0.5
        roi_img_pred_rec = roi_img_pred_rec / 2.0 + 0.5
        
        # calculate quality
        quality_dict_rec = self.quality_model(img_ori, img_rec)
        quality_dict_pred = self.quality_model(img_ori, img_pred_rec)
        quality_dict_roi = self.quality_model(roi_img, roi_img_pred_rec)
        ...
        result_dict = {
            ...
            # log ROI quality results
            "PSNR_ROI": quality_dict_roi['psnr'],
            "MS-SSIM_ROI": quality_dict_roi['msssim'],
            "LPIPS_ROI": quality_dict_roi['lpips'],
            "DISTS_ROI": quality_dict_roi['dists'],
