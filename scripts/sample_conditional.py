import argparse, os, sys, glob, math, time, importlib
import torch
import numpy as np
...
# import main.instantiate_from_config
from torch.utils.data import DataLoader
from torch.utils.data.dataloader import default_collate

# 定义一个lambda函数，用于将输入范围从[-1, 1]重新缩放到[0, 1]
rescale = lambda x: (x + 1.) / 2.


# 定义一个函数，用于将形状为BCHW（批量、通道、高度、宽度）的张量转换为ST（Streamlit）所需的格式
def bchw_to_st(x):
    return rescale(x.detach().cpu().numpy().transpose(0, 2, 3, 1))


def get_obj_from_str(string, reload=False):
    module, cls = string.rsplit(".", 1)  # 分割字符，此处分割为module和cls的字符串，例如taming.modules.transformer.mingpt， GPT
    if reload:
        module_imp = importlib.import_module(module)
        importlib.reload(module_imp)
    return getattr(importlib.import_module(module, package=None), cls)  # 获取指定模块‘module’中的cls属性


def instantiate_from_config(config):
    if not "target" in config:
        raise KeyError("Expected key `target` to instantiate.")
    return get_obj_from_str(config["target"])(**config.get("params", dict()))  # 创建‘target’的实例

# 定义一个函数，用于保存图像
def save_img(xstart, fname):
    # 将输入张量的值裁剪到[0, 1]范围内，乘以255并转换为uint8格式
    I = (xstart.clip(0,1)[0]*255).astype(np.uint8)
    # 通过Image库创建图像对象，并保存为指定文件名的图像文件
    Image.fromarray(I).save(fname)

# 定义一个函数，用于获取用户上传的交互式图像
def get_interactive_image(resize=False):
    # 使用Streamlit提供的文件上传器组件获取用户上传的图像文件
    image = st.file_uploader("Input", type=["jpg", "JPEG", "png"])
    if image is not None:
        # 打开上传的图像文件并转换为RGB格式的图像
        image = Image.open(image)
        if not image.mode == "RGB":
            image = image.convert("RGB")
        # 将图像转换为NumPy数组，并将数据类型转换为uint8
        image = np.array(image).astype(np.uint8)
        print("upload image shape: {}".format(image.shape))
        # 创建图像对象
        img = Image.fromarray(image)
        # 如果resize参数为True，则将图像大小调整为256x256像素
        if resize:
            img = img.resize((256, 256))
        # 将调整后的图像转换为NumPy数组并返回
        image = single_image_to_torch(img)
        return image
...
def single_image_to_torch(x, permute=True):
    ...
    # 将输入图像转换为NumPy数组
    x = np.array(x)
    # 将图像数据缩放到[-1, 1]范围内，并转换为PyTorch张量
    x = torch.FloatTensor(x/255.*2. - 1.)[None,...]
    # 如果permute参数为True，则对张量进行维度置换以匹配模型输入要求
    if permute:
        x = x.permute(0, 3, 1, 2)
    return x
...
def run_conditional(model, dsets):
    ...
    st.title("Image Charmer")
    st.subheader("By 中国传媒大学")
    ...
    upload = st.sidebar.checkbox("Upload your Image", value=False)

    if upload:
        x = get_interactive_image()
        if x is not None:
            x = x.to(model.device)
    else:
    ...
    image_compress = st.sidebar.checkbox("Image Compress/Sample Condition", value=False)

    if image_compress is True:
